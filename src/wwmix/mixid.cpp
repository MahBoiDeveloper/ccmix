...
class MixIdCodec
{
  public:
    ...
    static uint32_t DoBlock(const void *data, std::size_t size)
    {
        uint32_t rv = 0;
...
const uint32_t MixIdCodec::CrcTable[256] = {
...
int32_t IdGen(Game game, const std::string &fname_in)
{
    ...
    { // for TS
        ...
        return MixIdCodec::DoBlock(fname.c_str(), fname.length());
    }
}
...
{
    return fname.rfind(MixIdCodec::IdMarker, 0) == 0;
}
} // namespace MixId
