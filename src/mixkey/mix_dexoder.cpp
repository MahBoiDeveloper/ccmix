...
static uint32_t GetMulUint16(bignum n)
{
    ...
    wn = (uint16_t *)n;
    i = (((((((((*(wn - 1) ^ 0xffff) & 0xffff) * glob1_hi_inv_lo + 0x10000) >> 1) + (((*(wn - 2) ^ 0xffff) * glob1_hi_inv_hi + glob1_hi_inv_hi) >> 1) + 1) >> 16) + ((((*(wn - 1) ^ 0xffff) & 0xffff) * glob1_hi_inv_hi) >> 1) +
           (((*wn ^ 0xffff) * glob1_hi_inv_lo) >> 1) + 1) >>
          14) +
         glob1_hi_inv_hi * (*wn ^ 0xffff) * 2) >>
        glob1_hi_bitlen;
    if (i > 0xffff)
        i = 0xffff;
    return i & 0xffff;
    ...
    if (g2_len_x2 >= glob1_len_x2)
    {
        ...
        for (; len_diff != 0; len_diff--)
        {
            ...
            if (tmp > 0)
            {
                ...
                if ((*edi & 0x8000) == 0)
                {
                    if (SubBignum((uint32_t *)esi, (uint32_t *)esi, glob1, 0, len))
                        (*edi)--;
                }
            }
        }
